...
import (
	dpfm_api_input_reader "data-platform-api-inspection-lot-confirmation-doc-reads-rmq-kube/DPFM_API_Input_Reader"
	dpfm_api_output_formatter "data-platform-api-inspection-lot-confirmation-doc-reads-rmq-kube/DPFM_API_Output_Formatter"
	"fmt"
...
func (c *DPFMAPICaller) HeaderDoc(
	...
	if input.HeaderDoc.InspectionLot != nil {
		where = fmt.Sprintf("%s\nAND InspectionLot = %d", where, *input.HeaderDoc.InspectionLot)
	}
	...
	groupBy := "\nGROUP BY InspectionLot, Operations, OperationsItem, OperationID, ConfirmationCountingID, DocType, DocIssuerBusinessPartner "

	rows, err := c.db.Query(
		`SELECT
    	InspectionLot, Operations, OperationsItem, OperationID, ConfirmationCountingID, DocType, MAX(DocVersionID), DocID, FileExtension, FileName, FilePath, DocIssuerBusinessPartner
		FROM DataPlatformMastersAndTransactionsMysqlKube.data_platform_inspection_lot_confirmation_header_doc_data
		` + where + groupBy + `;`)
	if err != nil {
